...
function coerceValue(value, type, blameNode, path) {
  ...
  if ((0, _definition.isNonNullType)(type)) {
    if ((0, _isNullish2.default)(value)) {
      return ofErrors([coercionError('Expected non-nullable type ' + String(type), blameNode, path)]);
    }
  ...
  if ((0, _definition.isScalarType)(type)) {
    ...
    try {
      ...
      return ofErrors([coercionError('Expected type ' + type.name, blameNode, path, error)]);
    }
  }
  ...
  if ((0, _definition.isInputObjectType)(type)) {
    if ((typeof value === 'undefined' ? 'undefined' : _typeof(value)) !== 'object') {
      return ofErrors([coercionError('Expected object type ' + type.name, blameNode, path)]);
    }
    var _errors2 = void 0;
    var _coercedValue = {};
    ...
    for (var _fieldName in value) {
      if (hasOwnProperty.call(value, _fieldName)) {
        if (!fields[_fieldName]) {
          _errors2 = add(_errors2, coercionError('Field "' + _fieldName + '" is not defined by type ' + type.name, blameNode, path));
        }
      }
    }
...
function coercionError(message, blameNode, path, originalError) {
  ...
  return new _error.GraphQLError(message + (pathStr ? ' at ' + pathStr : '') + (originalError && originalError.message ? '; ' + originalError.message : '.'), blameNode, undefined, undefined, undefined, originalError);
}
